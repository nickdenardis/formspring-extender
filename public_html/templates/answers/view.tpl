...
<div class="box">
	<header>
		<h1>{$myObject->GetValue('question')|sslash}</h1>
	</header>
	<div class="box-content">
		<p>{$myObject->GetValue('answer')|sslash}</p>
		...
		{foreach from=$all_categories item="category"}
			<li><input type="checkbox"  name="category[]" value="{$category.category_id}"{if in_array($category.category_id, $in_categories)} checked="checked"{/if} /> {$category.category|sslash}</li>
		{/foreach}
		</ul>
